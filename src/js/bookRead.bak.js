...
$(function() {
    ...
    if(bookName == "") {
        ...
        $.ajax({
            type: "get",
            url: damApiHost + "book/text/" + topicId+"/"+userId,
            DataType: "json",
            async: true,
        ...
        $(".menu").hide();
        var liHtml = '';
        var damPtfUrl = getUrlParam("ptfUrl");
        //切换dam路径为代理路径，防止跨域
        ptfUrl = damPtfUrl.replace(damHost, damAgentHost);
        //添加数据
        $("#pdfViewer").attr('src', pdfHost + '?file=' + encodeURIComponent(ptfUrl) + "&page=1");
    ...
    var count = 0;	$.ajax({
        ...
        success: function(xml) {
            ...
            $(xml).find("book").each(function(i) {
                ...
                $(this).children().each(function(i) {
                    //单元文字查询
                    if($(this)[0].tagName == "title" || $(this)[0].tagName == "para" || $(this)[0].tagName == "author") {
